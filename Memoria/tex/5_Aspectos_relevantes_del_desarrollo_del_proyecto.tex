...
Para esta versión, la configuración necesaria de \textit{Maven} para que el proyecto compile es la siguiente \ruta{pom.xml}:
...
\subsubsection{Trabajo con \textit{streams} de \textit{Java}}
...
El paquete \textit{java.util.function} \footnote{\url{https://docs.oracle.com/javase/8/docs/api/java/util/function/package-summary.html}} es soportado por \textit{Java} desde la versión 1.8. Este paquete permite almacenar funciones en variables. 
...
\textit{Maven} es una herramienta de gestión de proyectos software. Esta herramienta facilita, a partir de un único fichero con extensión \textit{XML} llamado \ruta{pom.xml} \footnote{\textit{Project Object Model}}:
\begin{itemize}
	\tightlist
...
\imagen{M5_Eclipse_Maven}{Integración de \textit{Maven} con \textit{Eclipse}}
...
\begin{figure}[!h]
	\centering
	\includegraphics[width=0.7\textwidth]{M5_GitBash}
...
Cada mensaje generado con la clase \textit{Logger} debe tener asignado un nivel de importancia. Por ejemplo, se podrían plantear los siguientes niveles ordenador de mayor importancia a menos\cite{Level_class_info}:
\begin{description}
...
Como las dos herramientas tienen integración con \textit{Maven} sólo es necesario añadir en el fichero \ruta{pom.xml} las dependencias correspondientes en el fichero \ruta{pom.xml} del proyecto


Para poder añadir mensajes con el \textit{logger}, utilizamos la \textit{API SLF4J} (\ruta{slf4j-api}). Eso permite que si cambiáramos de \textit{Log4J} a otro \textit{logger} no tuviéramos que modificar el código si no sólo la conexión con la \textit{API SLF4J}.
...
Podemos ver la figura \ref{fig:M5_Action_secrets} cómo se almacenan los denominados \textit{secrets} en GitHub:
...
Para realizar las revisiones automáticas de cobertura, lo primero que hay que hacer es definir las pruebas utilizando \textit{JUnit} y ejecutarlas y posteriormente, utilizando \textit{JaCoCo} generaremos un informe donde se nos muestran los valores de cobertura del código.
...
\textit{GitHub} ofrece una \textit{API} \footnote{\url{https://docs.github.com/es/rest}} \textit{REST} \footnote{\textit{Representational state transfer}} para poder acceder a la información e interactuar con los repositorios alojados en \textit{GitHub}. Para interactuar con esta \textit{API} se ha utilizado la librería:\\
...
Como ejemplo del uso de \textit{Vaadin} en la aplicación podemos ver cómo para implementar el diálogo para cerrar las conexiones con las forjas de repositorios se emplea el siguiente código:
...
\begin{figure}[!h]
	\centering
	\includegraphics[width=1\textwidth]{M5_Close_Connection_Dialog}
	\caption{Diálogo con botones generado por \textit{Vaadin}}\label{fig:M5_Close_Connection_Dialog}
\end{figure}
\FloatBarrier

En la Fig. \ref{fig:M5_Comparativa_Repositorios} se muestra el diseño de la interfaz gráfica de la aplicación:

\begin{figure}[!h]
	\centering
	\includegraphics[width=1\textwidth]{M5_Comparativa_Repositorios}
