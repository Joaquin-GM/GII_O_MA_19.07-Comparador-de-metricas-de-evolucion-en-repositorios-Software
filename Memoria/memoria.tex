...
\textit{Evolution Metrics Gauge} es una aplicación web escrita en \textit{Java} con el \textit{framework Vaddin}. Toma como entrada un conjunto de repositorios públicos o privados y calcula métricas de evolución que permiten comparar los proyectos. 

El objetivo de este TFG es realizar un incremento funcional aplicando el flujo de desarrollo y despliegue continuo definido. En concreto, se actualizará y probará el desarrollo con una nueva versión de \textit{Vaadin}. En el incremento se replicará toda la funcionalidad para poder trabajar con la plataforma \textit{GitHub} y se integrará con la funcionalidad existente.   
Gracias a esto, con este TFG se ayuda a poder gestionar la calidad de los procesos de desarrollo con independencia de la forja utilizada, ya sea \textit{GitHub} o \textit{GitLab}.
